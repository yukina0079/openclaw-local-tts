...
如果你只是想本地快速生成一段语音，直接用：

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\speak.ps1 こんにちは 今日はいい天気です
```

它会：

- 直接调用 `tts-gptsovits.py`
- 使用当前配置里的默认模型 / 默认语言 / 默认 prompt
- 输出到：
...
- 让 `speak.ps1` 也彻底配置驱动，不走兜底路径

如果你现在点进这个仓库，它已经不只是“我电脑上的一坨脚本”，而是一个能在本地日常使用的 TTS 接线层了。